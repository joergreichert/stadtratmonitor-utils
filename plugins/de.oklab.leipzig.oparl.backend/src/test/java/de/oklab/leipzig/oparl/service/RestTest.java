...
public class RestTest {
	...
	public void testDownload() throws IOException {
		downloadFile(URI.create("https://mirror.oparl.org/bodies"));
	}
	...
	public void testParseBodyFromRest() throws IOException {
		...
		BodyResult result = restTemplate.getForObject(URI.create("https://mirror.oparl.org/bodies"), BodyResult.class);
	...
	public void testParseBodyFromFile() throws IOException {
		BodyResult result = new ObjectMapper().readerFor(BodyResult.class).readValue(new File("bodies.json"));
		for (Body data : result.getData().stream().filter(entry -> entry.getContactName().equals("Stadt Leipzig"))
